---
title: "Recreation of analysis for SURF conference 2023"
...
Now for the `n400_all` dataframe which is what we will be analysing, we need to  extract just the bins and channels that we intend to analyse. For this analysis we will use 9 channels:  F3 (3), Fz (2), F4(25),C3 (7), Cz (20), C4 (21), P3 (12), Pz (11), P4 (16).

```{r}
channels <-  c(3, 2, 25, 7, 20, 21, 12, 11, 16)
n400_subset <- n400_first |> filter(chindex %in% channels) 
...
```{r}
(responses_table <- table(n400_subset_oldnew$TrialType,
                          n400_subset_oldnew$ReadingProfile, 
...
pander(t.test(value ~ TrialType
       , filter(n400_subset_oldnew, ReadingProfile == "SEMANTIC" )
       , paired=TRUE))

pander(t.test(value ~ TrialType
       , filter(n400_subset_oldnew, ReadingProfile == "ORTHOGRAPHIC" )
       , paired=TRUE))
...
Plot Just the Reading Profile by Trial Type Interaction

```{r fig.dim = c(8, 8)}
library(ggplot2)
library(ggsci)
n400_right <- n400_subset_oldnew |> 
  group_by(ReadingProfile, TrialType) |>
  summarise(mean = mean(value), se = std.error(value))|> 
  mutate(lwr = mean - (1.96*se), upr = mean + (1.96*se))

(plot3<- ggplot(n400_left, aes(x = ReadingProfile, 
                         y = mean,
                         ymin = lwr,
                         ymax = upr,
                         fill = TrialType, 

...
bdf_file            = [DIR  filesep 'VSL_BDF_PART2.txt'];          % bin descriptor file in 'bdf' folder
og_setfile_path     = [DIR filesep 'm21_vsl_setfiles_202206'];     % folder containing original .set files that I'm starting with
new_setfile_path    = [DIR filesep 'm21_vsl_setfiles_202306'];     % folder for new .set files that I'm creating
erpfile_path        = [DIR filesep 'm21_vsl_erpfiles_202306'];
reref_file          = [DIR filesep 'reref_eq_brainvision.txt'];     % file with reref equations in 'ch_op' folder
subj_list           = readtable([DIR filesep 'm21_subjlist.csv']);  %list containing filenames to read it
...
for s=1:nsubj
    ...
    if (exist(setfilename, 'file')<=0|| exist(fdtfilename, 'file')<=0);
        ...
        EEG         = pop_binlister(EEG,...'BDF', bdf_file, ...
